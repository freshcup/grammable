...
RSpec.describe GramsController, type: :controller do
  ...
  describe "comments#create action" do
    it "should allow users to create comments on grams" do
      ...
      post :create, params: { gram_id: gram.id, comment: { message: 'hey there'} }

      expect(response).to redirect_to root_path
      expect(gram.comments.length).to eq 1
      expect(gram.comments.first.message).to eq "hey there"
    end
    ...
    it "should return http status code of not found if the gram isn't found" do
      user = FactoryBot.create(:user)
      sign_in user
      post :create, params: { gram_id: 'gram1', comment: { message: 'hey there'} }
	...
	describe "grams#destroy action" do
		...
		it "should return a 404 message if we cannot find a gram with the id that is specified" do
			user = FactoryBot.create(:user)
  			sign_in user
			delete :destroy, params: { id: 'MONKEY'}
			expect(response).to have_http_status(:not_found)
		end
	end
	...
	describe "grams#update action" do
		it "shouldn't let users who didn't create the gram update it" do
  			...
  			user = FactoryBot.create(:user)
  			sign_in user
  			patch :update, params: { id: gram.id, gram: { message: 'sweet' } }
  ...
  it "should have http 404 error if the gram cannot be found" do
    user = FactoryBot.create(:user)
    sign_in user
    patch :update, params: { id: "DUDE", gram: { message: 'Changed' } }
    expect(response).to have_http_status(:not_found)
  end
